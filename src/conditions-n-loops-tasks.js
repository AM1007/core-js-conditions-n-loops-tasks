...
function isContainNumber(num, digit) {
  const absoluteNum = num < 0 ? -num : num;

  if (absoluteNum === 0) {
    return digit === 0;
  }

  let tempNum = absoluteNum;
  while (tempNum > 0) {
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < n; i += 1) {
    for (let j = 0; j < n / 2; j += 1) {
      ...
      resMatrix[i][j] = resMatrix[i][n - 1 - j];
      resMatrix[i][n - 1 - j] = temp;
    }
  }
...
function shuffleChar(str, iterations) {
  ...
  do {
    ...
    tempStr = evenChars + oddChars;
    cycleLength += 1;
  } while (tempStr !== str && cycleLength < iterations);
  if (cycleLength < iterations) {
    remainingIterations = iterations % cycleLength;
  }
